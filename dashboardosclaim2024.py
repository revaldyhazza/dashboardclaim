...
fl = st.file_uploader("Upload ZIP that contains CSV File.", type="zip")
df = None  # inisialisasi

if fl is not None:
    zip_bytes = io.BytesIO(fl.read())
    
    with zipfile.ZipFile(zip_bytes) as zip_file:
        # ambil semua nama file dalam ZIP
        csv_files = [f for f in zip_file.namelist() if f.endswith(".csv")]

        if len(csv_files) == 0:
            st.warning("ZIP doesn't contain CSV File.")
        else:
            selected_csv = csv_files[0]

            with zip_file.open(selected_csv) as f:
                df = pd.read_csv(f)
    ...
    st.markdown("### 🎛️ Monthly Gross Claim by Business Summary")
    ...
    with col3:
        ...
        fig = px.pie(
            ...
            names="CoB", 
            hole=0.5,  # Membuat donut chart
            color_discrete_sequence=["#00205F", "#B3D1FF"]
        )
        fig.update_layout(
            width=400,  # Lebar keseluruhan
            height=250, # Tinggi keseluruhan
    ...
    # Agregasi data untuk pie chart (ambil 5 teratas)
    possible_region_cols = ["KC", "Region", "Region of Claim"]
    region_col = next((col for col in possible_region_cols if col in df5.columns), None)

    if region_col:
        KC_counts = df5[region_col].value_counts().nlargest(5).reset_index()
        KC_counts.columns = [region_col, "Count"]

        KC_sev = df5.groupby(region_col)["Gross Claim (In Million)"].sum().nlargest(5).reset_index()
        KC_sev.columns = [region_col, "Severity"]
    else:
        print("Tidak ditemukan kolom KC, Region, atau Region of Claim.")
    ...
    with col1:
        ...
        <h4 class="custom-title">Top 5 Frequency by Region/KC</h4>
        """, unsafe_allow_html=True)

        # Plotly Pie Chart untuk Frequency
        colors = ["#00205F", "#004080", "#0074CC", "#66A3FF", "#B3D1FF"]
        fig1 = px.pie(KC_counts, names=region_col, values="Count", hole=0.3, color_discrete_sequence=colors)
        fig1.update_layout(
            width=400,  # Lebar keseluruhan
            height=250, # Tinggi keseluruhan
            margin=dict(l=10, r=10, t=50, b=10),
    ...
    with col2:
        ...
        # Plotly Pie Chart untuk Severity
        colors = ["#00205F", "#004080", "#0074CC", "#66A3FF", "#B3D1FF"]
        fig2 = px.pie(KC_sev, names=region_col, values="Severity", hole=0.3, color_discrete_sequence=colors)
        fig2.update_layout(
            width=400,  # Lebar keseluruhan
            height=250, # Tinggi keseluruhan
            margin=dict(l=10, r=10, t=50, b=10),
    ...
    with col1:
        ...
        colors = [
            ...
            "#00cc00", "#33ff33", "#80ff80", "#ccffcc", "#ffffff"
        ]
